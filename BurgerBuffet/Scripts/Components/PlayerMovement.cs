...
public partial class PlayerMovement : Node2D
{
    ...
    {
        if (_currentDirection == direction.left) { _playerAnim.Play("Move_Left"); }
        if (_currentDirection == direction.right) { _playerAnim.Play("Move_Right"); }
        if (_currentDirection == direction.up) { _playerAnim.Play("Move_Up"); }
        if (_currentDirection == direction.down) { _playerAnim.Play("Move_Down"); }
    }
